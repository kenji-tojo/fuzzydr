# examples/07_fit_faces.py
#
# Multi-view inverse rendering with stochastic opacity masking: fit a set of
# unconnected triangles to renders of a reference mesh, and let the opacities
# decide how many survive.
#
...
# are deleted, and the longest of the survivors are split in two, from the midpoint of the
# longest edge to the opposite corner.
#
...
#   faces.npz        the fitted primitives (see common.save_checkpoint)
#   faces.obj        the same triangles, for a mesh viewer
#
# Usage:
#   python examples/07_fit_faces.py
#   python examples/07_fit_faces.py --iters 3000
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", type=str, default="out/07_fit_faces")
    ...
    ap.add_argument("--prim_size", type=float, default=0.02,
                    help="initial triangle radius, as a fraction of the "
                         "scene radius")
    ...
    init_verts, init_prims = common.make_random_triangles(
        args.count, camera.radius * args.prim_size, bbox_min, bbox_max)
    ...
    print(f"[faces] {primitives.count} primitives at opacity {args.opacity_init}, "
    ...
    def render(view, *, seed: int, tau: float = -1.0) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        rgba = fuzzydr.rasterize(
            ...
            faces=primitives.prims, face_opacity=opacity,
    ...
    def opacity_aux_loss(opacity: torch.Tensor, error: torch.Tensor) -> torch.Tensor:
        ...
        return fuzzydr.opacity_mask_aux_loss(face_opacity=opacity, error=error)
    ...
    try:
        ...
        common.save_obj_with_vertex_colors(
            out_dir / "faces.obj", primitives.verts.detach(),
            primitives.prims, primitives.colors.detach())
        ...
        print(f"faces: {args.count} -> {primitives.count}")
    finally:
        fuzzydr.shutdown()
