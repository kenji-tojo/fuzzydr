# examples/08_fit_lines.py
#
# Multi-view inverse rendering with stochastic opacity masking: fit a set of
# line segments to renders of a reference mesh, and let the opacities decide
# how many survive.
#
# The segments are drawn with bresen_lines=True, the 1-pixel LINE_LIST path,
# so the vertex radius slot is unread and no radius gradient is produced.
# Example 04 covers the quad line path, which does have a width.
#
...
# are deleted, and the longest of the survivors are split in two at their midpoints.
#
...
#   lines.npz        the fitted primitives (see common.save_checkpoint)
#
# Usage:
#   python examples/08_fit_lines.py
#   python examples/08_fit_lines.py --iters 3000
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", type=str, default="out/08_fit_lines")
    ...
    ap.add_argument("--count", type=int, default=300_000,
                    help="primitives to start from")
    ap.add_argument("--line_length", type=float, default=0.02,
                    help="initial segment length, as a fraction of the "
                         "scene radius")
    ...
    ap.add_argument("--budget", type=int, default=None,
                    help="primitive count to hold, refilled at most "
                         ...
                         "whatever pruning removes is put back by splitting")
    ...
    init_verts, init_prims = common.make_random_lines(
        args.count, camera.radius * args.line_length, bbox_min, bbox_max)
    ...
    print(f"[lines] {primitives.count} primitives at opacity {args.opacity_init}, "
    ...
    def render(view, *, seed: int, tau: float = -1.0) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        rgba = fuzzydr.rasterize(
            ...
            lines=primitives.prims, line_opacity=opacity,
    ...
    def opacity_aux_loss(opacity: torch.Tensor, error: torch.Tensor) -> torch.Tensor:
        ...
        return fuzzydr.opacity_mask_aux_loss(line_opacity=opacity, error=error)
    ...
    try:
        ...
        print(f"lines: {args.count} -> {primitives.count}")
    finally:
        fuzzydr.shutdown()
